...
└─────────────────────────────────────────────────┘
    ...
    ▼         ▼        ▼          ▼            ▼
┌─────────┐ ┌────┐ ┌──────┐ ┌─────────┐ ┌─────────┐
│Meal Plan│ │Dish│ │ Misc │ │Shopping │ │ History │
...
| **目的** | 新規アカウント作成 |
| **アクセス権限** | 未認証ユーザー |
| **主要機能** | - メールアドレス入力<br>- パスワード入力（確認含む）<br>- 登録ボタン<br>- ログインリンク |
...
| **遷移先** | - 詳細: `/dishes/{dish_id}`<br>- 新規: `/dishes/new` |
...
| **目的** | 料理の詳細表示・編集 |
| **アクセス権限** | 認証済みユーザー |
| **主要機能** | - 料理名表示<br>- 材料リスト表示<br>- カテゴリ表示<br>- 編集ボタン<br>- 削除ボタン<br>- 使用履歴（Nice to Have） |
| **遷移先** | - 編集: `/dishes/{dish_id}/edit`<br>- 削除: 確認ダイアログ → `/dishes` |
| **API** | GET `/api/v1/dishes/{dish_id}` |

#### SC-007: 料理登録・編集画面
| 項目 | 内容 |
|------|------|
| **URL** | `/dishes/new` または `/dishes/{dish_id}/edit` |
| **目的** | 料理の新規登録・編集 |
| **アクセス権限** | 認証済みユーザー |
| **主要機能** | - 料理名入力（必須）<br>- 材料リスト入力（複数行テキストエリア）<br>- カテゴリ選択<br>- 保存ボタン<br>- キャンセルボタン |
| **遷移先** | - 保存成功: `/dishes/{dish_id}`<br>- キャンセル: 前の画面 |
| **API** | POST `/api/v1/dishes` (新規)<br>PUT `/api/v1/dishes/{dish_id}` (編集) |
...
| **目的** | 登録済み雑貨の一覧表示 |
| **アクセス権限** | 認証済みユーザー |
| **主要機能** | - 雑貨リスト表示（リスト形式）<br>- カテゴリフィルタ（日用品/消耗品/その他）<br>- 検索バー<br>- 新規登録ボタン<br>- インライン編集・削除 |
| **遷移先** | - 新規: モーダル表示 |
| **API** | GET `/api/v1/miscellaneous` |
...
| **目的** | 雑貨の新規登録・編集 |
| **アクセス権限** | 認証済みユーザー |
| **主要機能** | - 雑貨名入力（必須）<br>- カテゴリ選択<br>- 保存ボタン<br>- キャンセルボタン |
| **遷移先** | - 保存成功: モーダル閉じる → リスト更新<br>- キャンセル: モーダル閉じる |
| **API** | POST `/api/v1/miscellaneous` (新規)<br>PUT `/api/v1/miscellaneous/{misc_id}` (編集) |
